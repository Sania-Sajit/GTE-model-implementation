...
def load_opus_hi_en_pairs(max_samples: Optional[int] = None, split: str = "train") -> List[MTPair]:
    ...
    print(f"Loaded {len(pairs):,} parallel pairs (en ↔ hi, split='{split}').")
    return pairs


def load_tatoeba_pairs(max_samples: Optional[int] = None, lang_pair: str = "en-de", split: str = "train") -> List[MTPair]:
    """
    Load parallel sentences from Tatoeba / OPUS corpora.
    Supports lang_pair='en-de' or 'en-hi', and split='train' or 'test'.
    """
    if lang_pair == "en-hi":
...
def build_mt_dataloader(
    ...
    """Build PyTorch DataLoader for MT fine-tuning."""
    pairs = load_tatoeba_pairs(max_samples=max_samples, lang_pair=lang_pair)
